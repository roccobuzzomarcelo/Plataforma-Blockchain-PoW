...
// Usando uint32_t en lugar de uint64_t para evitar
// multiplicaciones de 64 bits no soportadas por GPGPU-Sim
...
__global__ void bruteForceKernel(
    ...
    // uint32_t para evitar multiplicaciones de 64 bits
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("Uso: %s <cadena> <prefijo>\n", argv[0]);
        printf("Ejemplo: %s \"blockchain\" \"00\"\n", argv[0]);
        return 1;
    }
    ...
    printf("\n");
    printf("Verificacion:     Hash comienza con \"%s\" ✓\n", prefix);
